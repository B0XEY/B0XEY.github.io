...
| **Depth** | Just for 3D. How many Z slices, 1 to 4096 |
...
| **Seamless (Tileable)** | Just for 2D. Makes the texture wrap with no seam on both axes |
| **Gradient** | Colours the result. Defaults to black-to-white, **Reset** puts it back |
| **Format** | Just for 2D. Texture asset, PNG, or EXR |

### Seamless

Ticked, Noizy uses FastNoise2's tileable generator, so the texture wraps
with no seam on both axes. That's what you want for a repeating material.
...
through the gradient. So a graph that just outputs -0.2 to 0.3 still uses
...
- For 3D, the window shows a live voxel count and a rough memory estimate as you
  change settings.
- Above around 64 million voxels (about 400x400x400) it asks you to confirm
...
**The preview can look a little different from a 3D export.** The preview
...
The exporter is an editor tool. If you want textures at runtime, sample the graph
